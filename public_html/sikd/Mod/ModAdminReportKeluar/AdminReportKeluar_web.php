	...
	<div class="content-main-report">
		<table width="100%" cellpadding="0" cellspacing="0">
			<tr>
				<td style="text-align:center;">
					<h2><b>LAPORAN NASKAH KELUAR</b></h2>
				</td>
			</tr>
			<tr>
				<td style="text-align:center;">
		...
		<table width="100%" class="tb_report" cellspacing="0" style="border:#000000 1px solid;">
			<tr>
				...
				<th width="8%" style="padding:0.4em; border-right: #000000 solid 1px;border-bottom: #000000 solid 1px;">Tanggal Naskah Keluar</th>
            ...
            $sql = "SELECT 
                    ...
                    inbox.Hal,
                    role.RoleName,
                    inbox_receiver.RoleId_From,
                    inbox.pengirim,
                    (case inbox_receiver.ReceiverAs
                      when 'to' then 'Naskah Masuk Eksternal'
                      when 'to_tl' then 'Naskah Tanpa Tindaklanjut'
                      when 'to_konsep' then 'Nota Dinas'
                      when 'to_notadinas' then 'Nota Dinas'
                      when 'to_memo' then 'Memo'
                      when 'to_forward' then 'Teruskan'
                      when 'to_reply' then 'Nota Dinas'
                      when 'to_usulan' then 'Nota Dinas'
                      when 'to_keluar' then 'Konsep Naskah Keluar'
                      when 'cc1' then 'Disposisi' else
                      'Surat Masuk Internal' end) as Ket,
					inbox.NTglReg, inbox_receiver.ReceiveDate
                    FROM
                    inbox
                    Inner Join inbox_receiver ON inbox_receiver.NId = inbox.NId
                    Inner Join role ON role.RoleId = inbox_receiver.RoleId_To
                    WHERE RoleId_From = '$_SESSION[PrimaryRoleId]' and ";

            if(!empty($tgl1) and !empty($tgl2))
            $sql .= "(inbox_receiver.ReceiveDate between '" . mkdate($tgl1) . "' and '" . mkdate($tgl2) . "') ";

            $sql .= ' order by inbox_receiver.ReceiveDate DESC ';
			$res = mysql_query($sql);
			$no=1;
			...
			while($row = mysql_fetch_array($res)){
			  ...
			  ?>
                    ...
                    <?php
                        $ssl = mysql_query("select rolename from role where roleid = '".$row['RoleId_From']."'");
                    ...
                    <td style="padding:0.4em; border-right: #000000 solid 1px;border-bottom: #000000 solid 1px;"><center><?php echo tgl_indo($row["ReceiveDate"]); ?></center></td>
