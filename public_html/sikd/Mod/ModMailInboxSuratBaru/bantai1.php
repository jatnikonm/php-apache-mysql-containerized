...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where RoleId_To = '$_SESSION[PrimaryRoleId]' and StatusReceive = 'unread' group by NId
			$sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere AND RoleId_To = '$_SESSION[PrimaryRoleId]' and StatusReceive = 'unread' group by NId  
			$sOrder
			$sLimit
			";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'StatusReceive' )
            {
				...
				if ($StatusReceive == 'read')
				{
					...
					else 
					{
						...
						if($res[StatusReceive] == 'unread'){
							$row[] .= ''."<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'><b>Belum Dibaca</b></a>".'';				 
						}else{
            ...
            elseif ( $aColumns[$i] == 'InstansiPengirim' )
            {
				...
				if($res[StatusReceive] == 'unread'){
					...
					if($hasil2221[NTipe]=='outboxnotadinas')
					{
						...
						$row[] .= "<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'><b>$hasil222[RoleName]</b></a>";
					}
					...
					{
						$row[] .= "<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'><b>$InstansiPengirim</b></a>";
					}
				}else{
					$cari21=mysql_query("select NTipe from inbox where NId = '$NId'");
					$hasil2221=mysql_fetch_array($cari21);
					...
					if($hasil2221[NTipe]=='outboxnotadinas')
					{
						...
						$row[] .= "<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'>$hasil222[RoleName]</a>";
					}
					elseif($hasil2221[NTipe]=='outboxmemo')
					{
						$cari2=mysql_query("select RoleName from role where RoleId = '$InstansiPengirim'");
            ...
            elseif ( $aColumns[$i] == 'Hal' )
            {
				...
				if($res[StatusReceive] == 'unread'){
					if($res2[BerkasId] == '1'){
						$row[] .= ''."<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'><b>$Hal <br><font color=red>(Belum Diberkaskan)</font></b></a>".'';				 
					}else{
            ...
            elseif ( $aColumns[$i] == 'ReceiveDate' )
            {
				...
				if($res[StatusReceive] == 'unread'){
					$row[] .= ''."<a href=index2.php?option=MailTL&id=$NId style='text-decoration:none'><b>$ReceiveDate</b></a>".'';				 
				}else{
