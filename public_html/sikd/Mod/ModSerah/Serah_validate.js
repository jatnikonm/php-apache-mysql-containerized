	...
	function setDetails(cond, id){
	  if(cond=="new"){
       ...
       ltblberkas("",task);
        ...
        $.ajax({
        url: "Mod/ModSerah/load_surat.php", // Url to which the request is send
...
//        }
      ...
      if(validasi){
          if($("#task").val()=='new'){
           tanya = confirm("Apakah berita acara dan usulan penyerahan Arsip Statis sudah yakin... !!");
          }else tanya = true;
           if(tanya)
    ...
    function set_surat(){
      ...
      $.ajax({
        url: "Mod/ModSerah/load_berkas.php", // Url to which the request is send
        type: "POST",             // Type of request to be send, called as method
