	...
	<?php
		...
		//-----------------------------------------------------------------------------------
		 ...
		 if ($_SESSION["GroupId"] == "6"){
			$sql .= " 	and ir.RoleId_To = '" . $_SESSION["RoleAtasan"] . "' 
						and ir.NId = '" . $id . "' ";
		 } else {
			$sql .= " 	and ir.RoleId_To = '" . $_SESSION["PrimaryRoleId"] . "' 
						and ir.NId = '" . $id . "' ";
		 }
				...
				if ($row["ReceiverAs"] == "bcc_HA"){
					...
					//detect if User had Follower, if any -- for Disposisi purpose
					 ...
					 if ($_SESSION["GroupId"] == "6"){
						$sql2 = "select * 
								from role 
								where RoleId like '" . $_SESSION["RoleAtasan"] . ".%'";
					 } else {
						$sql2 = "select * 
								from role 
					...
					<script type="text/javascript" charset="utf-8">
                        $(document).ready(function() {
                            $('#example212').dataTable({
                                ...
                                "bServerSide": true,
                                "aaSorting": [[1, "desc"]],
                                "sAjaxSource": "Mod/ModMailTLPimpinan/bantai2.php?id=<?php echo "$_GET[id]"; ?>"
                            });
					...
					<script type="text/javascript" charset="utf-8">
                        $(document).ready(function() {
                            $('#example21').dataTable({
                                ...
                                "bServerSide": true,
                                "aaSorting": [[1, "desc"]],
                                "sAjaxSource": "Mod/ModMailTLPimpinan/bantai1.php?id=<?php echo "$_GET[id]"; ?>"
                            });
