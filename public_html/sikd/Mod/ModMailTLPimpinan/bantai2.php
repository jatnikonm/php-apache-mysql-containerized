	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'RoleId_From2' )
            {
				...
				if(($hasil2221[RoleId_From] == $RoleId_From2) and ($hasil2221[RoleId_From] == $_SESSION[RoleAtasan]) and ($hasil2221[ReceiveDate] == $waktu1)) {
					$row[] .= ''."<a href=handle.php?option=MailTLPimpinan&task=deleteTL&NId=$_GET[id]&GIR_Id=$GIR_Id onclick=\"return confirm('Apakah Akan Membatalkan Pengiriman Surat ??')\"><center><img src=images/delete.png title='Batalkan Pengiriman Surat'></center></a>".' ';
				}
				else
				{
					$row[] .= ''."".'';
