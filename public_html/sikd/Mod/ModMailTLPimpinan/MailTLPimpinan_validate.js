...
$('#tab6').ready(function() {
    $('#example').dataTable({
		...
		"bServerSide": true,
		"aaSorting": [[4, "desc"]],
		"sAjaxSource": "Mod/ModMailTLPimpinan/json.php"
    });
...
$('#example tbody').on( 'click', 'tr', function () {
  ...
  // ... do something with `rowData`
        $msg = confirm("Apakah naskah ingin diberkaskan ulang di berkas : "+rowData[2]+" - "+rowData[3]+" ?");
        if($msg)
        {
          $.ajax ({
              url:"Mod/ModMailTLPimpinan/MailTLPimpinan_proses.php",
              type: "POST",
 ...
 /* --------------------------------------------------- */
	...
	function openRD(str){
		...
		method.value = 'new';
		getWindow('mailTL_RD2', '&task=new' + str + '&modeRD=' + str + '&NId=' + NId);
	}
	...
	function openRDEdit(str, id){
		...
		switch(str){
			case "final":
				...
				break;
			default:
				getWindow('mailTL_RD2', '&task=edit' + str + '&modeRD=' + str + '&NId=' + NId + '&GIR_Id=' + id);	
				break;
		}		
	...
	function openRDPrint(Nid, Gid){
		//var NId = document.forms.form1.id.value;
		openWindow('mailTL_Disp_print2', 'NId=' + Nid + '&GIR_Id=' + Gid );
	}
	...
	function getSelectRD(mode, val){
		...
		getWindow('mailTL_RD2', '&task=' + method + str + '&modeRD=' + str + '&NId=' + NId + '&GIR_Id=' + GIR_Id + '&peopleIDKepada=' + peopleIdTo.value + '&peopleIDCC=' + peopleIdCC.value);
	...
	function deleteHistory(nid, gid){
		var conf = window.confirm('Apakah Anda Akan Membatalkan Tindaklanjut Naskah Ini ?');
		if(conf == true){
			var task = 'deleteTL';
