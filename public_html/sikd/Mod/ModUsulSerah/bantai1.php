...
session_start();
	 ...
	 */
	$aColumns = array( 'idserah', 'nomor', 'tgl', 'UploadSurat', 'ket' );

	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "idserah";

	/* DB table to use */
	$sTable = "permohonan_serah";
	...
	if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
	{
		...
		$sQuery = "
			SELECT idserah, nomor, tgl, ket, UploadSurat
			FROM   $sTable
			$sWhere
			$sOrder
			$sLimit
	...
	{
	   $xml = "Select PermohonanId from berita_acara where PermohonanId = '".$aRow[0]."' and stat = 's'";
       ...
       }else
       {
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            if($temu!=0) $ket = "Serah"; else $ket = "Usulan";
  ...
  <a target="_blank" href ="Mod/ModUsulSerah/daftarberkas_web.php?mod=um&ids='.$aRow[0].'" ><i class="btn_print" style="cursor:pointer" title="Cetak Daftar"></i></a>
  </div>';
