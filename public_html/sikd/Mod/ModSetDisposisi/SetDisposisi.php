...
<script type="text/javascript">	
	document.getElementById("title").innerHTML = 'Pengaturan Umum -> Isi Disposisi';
</script>
...
<div class="content-main-popup">
	<div id="pnlDetails" style="display:none">
		<table style="width:100%;" class="tb_grid" cellpadding="2" cellspacing="0">
			<tr>
				<td style="width:15%;"> Isi Disposisi <font color="red">*</font></td>
				<td style="width:85%;">
					<input type="text" name="txt1" class="inputbox" style="width:70%;">&nbsp;
            ...
            <tr> 
                ...
                <td>
                    <select name="txt2" class="inputbox">
                        <option value="">-</option>
                        <?php 
                            $sql = "select gjabatanId, gjabatanName from master_gjabatan ";
                            $sql .= "order by gjabatanName";
                            $result = mysql_query($sql);
                            while($row = mysql_fetch_array($result)){
								echo "<option value='" . $row["gjabatanId"] . "'";
								if($data[2] == $row["gjabatanId"]){
									echo "selected";
								}
								echo ">" . $row["gjabatanName"] . "</option>";
                            }
			...
			<tr>
				...
				<td>
					...
					<input type="hidden" name="count" value="2" />  <!-- fungsi count untuk menentukan jumlah textfieldnya -->
	...
	<div id="pnlGrid">
	<?php
		$sql = "select j.gjabatanName, d.* from master_disposisi d ";
		$sql .= " left join master_gjabatan j on j.gjabatanId = d.gjabatanId ";
	...
	<table class="adminlist" style="width:100%" cellpadding="2" cellspacing="0" >
		<tr>
			...
			<th style="width:1%">&nbsp;</th>
            <th style="width:15%">Grup Jabatan</th>
			<th style="width:87%">Isi Disposisi</th>
		</tr>
		<?php
			$no = $startrec;
			for ($i = $startrec; $i < $reccount; $i++)
			{
				...
				<tr style="background-color:<?php echo $bg; ?>">
					<td><?php echo $no; ?></td>
					<td><input type="checkbox" name="ids[]" value="<?php echo $row["DisposisiId"]; ?>" /></td>
					<td><a href="#" onclick="setDetails('edit','<?php echo $row["DisposisiId"] . '|' . $row["DisposisiName"] . '|' . $row["gjabatanId"]; ?>')"><img src="images/edit.png" border="0" /></a></td>
					<td><?php echo $row["gjabatanName"]; ?></td>
					<td><?php echo $row["DisposisiName"]; ?></td>
				</tr>
				<?php							
			}					
			mysql_free_result($result);
