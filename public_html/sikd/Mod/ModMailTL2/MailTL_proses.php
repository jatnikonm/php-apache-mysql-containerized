...
session_start();
    ...
    if($task=="berkas"){
		...
		$Bid = $_REQUEST["Bid"];
		include ("../../conf.php");
		$sql  = "Select BerkasId from Berkas Where BerkasNumber = '".$Bid."'";
	...
	if($task == "deleteTL"){
		...
		$tampil1 = mysql_query("select NFileDir from inbox where NId = '$NId'");
		$hasil1 = mysql_fetch_array($tampil1);
		...
		while($row = mysql_fetch_array($res)){
			...
			if(file_exists('FilesUploaded/' . $hasil1[NFileDir] . '/' . $files)){
				unlink('FilesUploaded/' . $hasil1[NFileDir] . '/' . $files);
			}
		}
		//----------------------------------------------------------------
		...
		$location = "alert('" . $msg . " !');
					location.href='index2.php?option=MailTL&task=detail&id=" . $NId . "';";
	}
