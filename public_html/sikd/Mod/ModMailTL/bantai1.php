	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			elseif ( $aColumns[$i] == 'RoleId_To' )
        	{
				...
				{
					$cari21=mysql_query("SELECT (GROUP_CONCAT(rr.RoleName ORDER BY rr.RoleId ASC SEPARATOR ', ')) as penerima
										FROM inbox_receiver irr 
										join role rr on rr.RoleId = irr.RoleId_To
										where irr.NId = '" . $_GET["id"] . "' and irr.GIR_Id = '$GIR_Id'
										and irr.ReceiverAs in ('to', 'to_tl', 'to_keluar', 'to_notadinas', 'to_konsep', 'to_memo', 'cc1', 'to_usul', 'to_reply', 'to_forward') 
										GROUP BY irr.GIR_Id");
					$hasil222=mysql_fetch_array($cari21);
					
					$cari212=mysql_query("SELECT (GROUP_CONCAT(rr.RoleName ORDER BY rr.RoleId ASC SEPARATOR ', ')) as tembusan
										FROM inbox_receiver irr 
										join role rr on rr.RoleId = irr.RoleId_To
										where irr.NId = '" . $_GET["id"] . "' and irr.GIR_Id = '$GIR_Id'
            ...
            elseif ( $aColumns[$i] == 'Msg' )
            {
				...
				$Msg1 = "<table style='width:100%;' cellpadding='0' cellspacing='0'>";
				$Msg1 .=  "<tr><td>$Msg</td></tr>";
				$Msg1 .=  "</table>";
				...
				$cari212x=mysql_query("select NFileDir from inbox where NId = '" . $_GET["id"] . "'");
				$hasil2222x=mysql_fetch_array($cari212x);

				$files_Attc = "";
				//File Attachment
				//--------------------- for displaying files attachment if any -----------------------------
				$sql_files = "select concat('FilesUploaded/','" . $hasil2222x["NFileDir"] . "','/',infs.FileName_fake) as FileDisk, ";
				$sql_files .= "infs.FileName_real "; 			
				...
				if(mysql_num_rows($res_files) > 0) {
				    ...
				    if($hasil2221[x]=='to'){
							$NId = $_GET["id"];
							$files_Attc = ''."<input id=mylink title='Upload File Digital' type=button name=btnFinal style=cursor:pointer value='Upload File' class=btn_view_final onclick=uploadfile('".$NId."','".$GIR_Id."'); />";
					}
                }
				$row[] .= ''."$disp $Msg1 $files_Attc".'';
			}		
