...
$('#example tbody').on( 'click', 'tr', function () {
  ...
  // ... do something with `rowData`
        $msg = confirm("Apakah surat ingin diberkaskan ulang di berkas : "+rowData[2]+" - "+rowData[3]+" ?");
        if($msg)
        {
          $.ajax ({
 ...
 /* --------------------------------------------------- */
    ...
    function uploadfile(NId, GIR_Id){
      //alert(GIR_Id);
      getWindow('uploadfile','&task=new&NId='+NId+'&GIR_Id='+GIR_Id);
    }	
