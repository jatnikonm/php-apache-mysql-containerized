...
session_start();
	 ...
	 */
	$aColumns = array( 'NId', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where CreationRoleId = '$_SESSION[PrimaryRoleId]' and NTipe = 'inboxuk' group by NId 
			$sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere AND CreationRoleId = '$_SESSION[PrimaryRoleId]' and NTipe = 'inboxuk' group by NId   
			$sOrder
			$sLimit
			";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'InstansiPengirim' )
            {
				...
				$cari2=mysql_query("select * from inbox_receiver where NId = $NId and ReceiverAs in ('cc1', 'to_reply', 'to_usul', 'to_forward')");
				$hasil222=mysql_fetch_array($cari2);

				if ($hasil222[NId] == "")  
				{  
					$row[] .= ''."<a href=handle.php?option=MailInboxUk&task=delete&ids=$NId onclick=\"return confirm('Apakah Surat Benar-Benar Akan Dihapus??')\"><center><img src=images/delete.png title='Hapus Surat'></center></a>".' ';
				}  
				else 
				{
					$InstansiPengirim = $aRow[ $aColumns[$i] ];
