...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where CreationRoleId = '$_SESSION[PrimaryRoleId]' and NTipe = 'outboxins' group by NId 
			$sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere AND CreationRoleId = '$_SESSION[PrimaryRoleId]' and NTipe = 'outboxins' group by NId   
			$sOrder
			$sLimit
			";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'zz' )
            {
				...
				if ($hasil222[NId] == "")  
				{  
					$row[] .= ''."<a href=handle.php?option=LogIns&task=delete&ids=$NId onclick=\"return confirm('Apakah Surat Benar-Benar Akan Dihapus??')\"><center><img src=images/delete.png title='Hapus Surat'></center></a>".' ';
				}  
				else 
				{
					$row[] .= ''.' ';
