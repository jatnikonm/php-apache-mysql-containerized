	...
	if($task == "new" ||
		...
		$sql = "select * from berkas 
				where BerkasKey = '" . $_SESSION["AppKey"] . "' 
					and BerkasNumber = '" . trim($data[4]) . "'
                    and Klasifikasi = '" . trim($data[3]) . "'";
		$res = mysql_query($sql);
		if(mysql_num_rows($res) > 0 ){
		    $numberkas = $data[4]++;
			echo "<script>alert('Data dengan Nomor Berkas " . trim($data[3]) . trim($data[4]) . " sudah ada , Nomor Berkas diubah menjadi : ".$numberkas." !');</script>";
