...
session_start();
	 ...
	 */
	$aColumns = array( 'BerkasId', 'BerkasStatus', 'BerkasNumber', 'BerkasName', 'RetensiValue_Active' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where RoleId = '$_SESSION[PrimaryRoleId]'
			$sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere AND RoleId = '$_SESSION[PrimaryRoleId]'  
			$sOrder
			$sLimit
			";
	}
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
            ...
            elseif ( $aColumns[$i] == 'BerkasNumber' )
            {
				...
				$cari2=mysql_query("select Klasifikasi from berkas where BerkasId = '$BerkasId'");
				$hasil222=mysql_fetch_array($cari2);

            	$BerkasNumber = $hasil222['Klasifikasi']."/".$aRow[ $aColumns[$i] ];
