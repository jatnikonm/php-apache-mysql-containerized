<?php
	if ($task == "new"){
		try {
            ...
            for($f=1;$f<count($arr_id);$f++)
            {
                 ...
                 $ddr = "update berkas set KetAkhir = 'musnah' where BerkasId ='".$arr_id[$f]."' ";
                 mysql_query($ddr);
                 ...
                 while ($fetch = mysql_fetch_array($getddit)){
                   ...
                   $sql_g = "Update inbox_files set Keterangan = 'musnah' where inbox_files.NId = '".$fetch[Nid]."'";
                   mysql_query($sql_g);
	...
	if ($task == "delete"){
		$idnomor = $_REQUEST["id"];
		try {
		    $sql = "select UploadSurat from berita_acara where BerkasId = '$idnomor'";
            $path = mysql_fetch_array(mysql_query($sql));
			$sql = "delete from berita_acara ";
			$sql .= "where BerkasId = '$idnomor' ";
			mysql_query($sql);
            unlink($path[0]);
