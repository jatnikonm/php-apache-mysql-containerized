...
session_start();
	 ...
	 */
	$aColumns = array('BeritaId', 'Tgl', 'UploadSurat');
	...
	$sWhere = "where berita_acara.stat='m'";
	...
	if ( isset($_GET['sSearch']) && $_GET['sSearch'] != "" )
	{
		...
		$sQuery = "
			SELECT berita_acara.BeritaId, berita_acara.Tgl, berita_acara.UploadSurat, permohonan_musnah.nomor, permohonan_musnah.tgl, permohonan_musnah.UploadSurat, berita_acara.Nomor,
            permohonan_musnah.idmusnah
            FROM
            berita_acara
            Inner Join permohonan_musnah ON berita_acara.PermohonanId = permohonan_musnah.idmusnah
			$sWhere
			$sOrder
			$sLimit
			";
  ...
  <a target="_blank" href ="Mod/ModMusnah/daftarberkas_web.php?ids='.$aRow[7].'" ><i class="btn_print" style="cursor:pointer;display:'.$display.'" title="Cetak Berkas"></i></a>
  </div>';
